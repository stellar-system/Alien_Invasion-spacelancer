...
def game_begin(settings, sb, stats, aliens, bullets, screen, ship):
    # 重置游戏设置
...
def check_keydown_events(event, settings, screen, ship, bullets, stats, sb, aliens):
    ...
    elif not stats.game_active and event.key == pygame.K_SPACE:
        game_begin(settings, sb, stats, aliens, bullets, screen, ship)
...
def update_screen(settinfs, screen, stats, sb, ship, aliens, bullets, play_button, background1, background2, background_y, scroll_speed):
    """更新屏幕上的图像，并切换到新屏幕"""
    # 每次循环时都重绘屏幕
